...
export type CoordinatedPaperScene = {
  ...
  /** Place the camera on a sphere around the phone (degrees). */
  setCameraOrbit: (azimuth: number, elevation: number) => void;
  /** True once the phone model has loaded and the scene can render it. */
  isLoaded: () => boolean;
...
export function mountCoordinatedPaperScene(
  ...
  return {
    ...
    setCameraOrbit(azimuth, elevation) {
      const az = THREE.MathUtils.degToRad(azimuth);
      const el = THREE.MathUtils.degToRad(elevation);
      camera.position.set(10 * Math.cos(el) * Math.sin(az), 10 * Math.sin(el), 10 * Math.cos(el) * Math.cos(az));
      camera.lookAt(0, 0, 0);
      controls.target.set(0, 0, 0);
      controls.update();
      render();
